...
interface AnimatedAvatarProps {
  emailInputId?: string;
  /** Omit this (or pass undefined) on screens with no password field, e.g. Forgot Password */
...
export const AnimatedAvatar = ({
  ...
  const mouthBG = useRef<SVGPathElement>(null);
  const mouthSmallBG = useRef<SVGPathElement>(null);
  const mouthMediumBG = useRef<SVGPathElement>(null);
  const mouthLargeBG = useRef<SVGPathElement>(null);
  const mouthMaskPath = useRef<SVGPathElement>(null);
  const mouthOutline = useRef<SVGPathElement>(null);
  const tooth = useRef<SVGPathElement>(null);
  const tongue = useRef<SVGGElement>(null);
  const chin = useRef<SVGPathElement>(null);
  const face = useRef<SVGPathElement>(null);
  const eyebrow = useRef<SVGGElement>(null);
  const outerEarL = useRef<SVGGElement>(null);
  const outerEarR = useRef<SVGGElement>(null);
  const earHairL = useRef<SVGGElement>(null);
  const earHairR = useRef<SVGGElement>(null);
  const hair = useRef<SVGPathElement>(null);

  const mouthStatus = useRef<'small' | 'medium' | 'large'>('small');
  // tracks whether eyes are currently covered so focus/blur can't fight itself
  const eyesCovered = useRef(false);

  useEffect(() => {
    const email = document.getElementById(emailInputId) as HTMLInputElement | null;
    const password = passwordInputId
      ? (document.getElementById(passwordInputId) as HTMLInputElement | null)
      : null;
    const mySVG = svgContainerRef.current;

    // Only email + the SVG container are required. Password is optional
    // (e.g. the Forgot Password screen has no password field at all).
    ...
    function getCoord() {
      ...
      const dFromC = getPosition(mySVG).x + mySVG.offsetWidth / 2 - (emailCoords.x + caretCoords.x);
      ...
      // Don't animate the eyes/face while the arms are covering them —
      // otherwise the eye tween fights the "peeking" pose visually.
      if (!eyesCovered.current) {
        gsap.to(eyeL.current, { duration: 1, x: -eyeLX, y: -eyeLY, ease: 'expo.out', overwrite: 'auto' });
        gsap.to(eyeR.current, { duration: 1, x: -eyeRX, y: -eyeRY, ease: 'expo.out', overwrite: 'auto' });
      }
      gsap.to(nose.current, { duration: 1, x: -noseX, y: -noseY, rotation: mouthR, transformOrigin: 'center center', ease: 'expo.out', overwrite: 'auto' });
      gsap.to(mouth.current, { duration: 1, x: -mouthX, y: -mouthY, rotation: mouthR, transformOrigin: 'center center', ease: 'expo.out', overwrite: 'auto' });
      gsap.to(chin.current, { duration: 1, x: -chinX, y: -chinY, scaleY: chinS, ease: 'expo.out', overwrite: 'auto' });
      gsap.to(face.current, { duration: 1, x: -faceX, y: -faceY, skewX: -faceSkew, transformOrigin: 'center top', ease: 'expo.out', overwrite: 'auto' });
      gsap.to(eyebrow.current, { duration: 1, x: -faceX, y: -faceY, skewX: -eyebrowSkew, transformOrigin: 'center top', ease: 'expo.out', overwrite: 'auto' });
      gsap.to(outerEarL.current, { duration: 1, x: outerEarX, y: -outerEarY, ease: 'expo.out', overwrite: 'auto' });
      gsap.to(outerEarR.current, { duration: 1, x: outerEarX, y: outerEarY, ease: 'expo.out', overwrite: 'auto' });
      gsap.to(earHairL.current, { duration: 1, x: -outerEarX, y: -outerEarY, ease: 'expo.out', overwrite: 'auto' });
      gsap.to(earHairR.current, { duration: 1, x: -outerEarX, y: outerEarY, ease: 'expo.out', overwrite: 'auto' });
      gsap.to(hair.current, { duration: 1, x: hairX, scaleY: 1.2, transformOrigin: 'center bottom', ease: 'expo.out', overwrite: 'auto' });
    ...
    function resetFace() {
      if (eyesCovered.current) return; // don't fight the covered pose
      gsap.to([eyeL.current, eyeR.current], { duration: 1, x: 0, y: 0, ease: 'expo.out', overwrite: 'auto' });
      gsap.to(nose.current, { duration: 1, x: 0, y: 0, ease: 'expo.out', overwrite: 'auto' });
      gsap.to(mouth.current, { duration: 1, x: 0, y: 0, rotation: 0, ease: 'expo.out', overwrite: 'auto' });
      gsap.to(chin.current, { duration: 1, x: 0, y: 0, scaleY: 1, ease: 'expo.out', overwrite: 'auto' });
      gsap.to([face.current, eyebrow.current], { duration: 1, x: 0, y: 0, skewX: 0, ease: 'expo.out', overwrite: 'auto' });
      gsap.to([outerEarL.current, outerEarR.current, earHairL.current, earHairR.current, hair.current], {
        duration: 1, x: 0, y: 0, scaleY: 1, ease: 'expo.out', overwrite: 'auto',
      });
    }

    // Single source of truth for the arm state. Kills any in-flight arm
    // tweens before starting the new one, so rapid focus/blur (autofill,
    // fast tabbing) can't leave the arms stuck mid-animation or fighting
    // between "covering" and "uncovering".
    function coverEyes() {
      eyesCovered.current = true;
      gsap.killTweensOf([armL.current, armR.current]);
      const tl = gsap.timeline();
      tl.to(armL.current, { duration: 0.45, x: -93, y: 2, rotation: 0, ease: 'power2.out', overwrite: 'auto' }, 0);
      tl.to(armR.current, { duration: 0.45, x: -93, y: 2, rotation: 0, ease: 'power2.out', overwrite: 'auto' }, 0.1);
    }
    ...
    function uncoverEyes() {
      ...
      gsap.killTweensOf([armL.current, armR.current]);
      const tl = gsap.timeline();
      tl.to(armL.current, { duration: 1.35, y: 220, rotation: 105, ease: 'power2.out', overwrite: 'auto' }, 0);
      tl.to(armR.current, { duration: 1.35, y: 220, rotation: -105, ease: 'power2.out', overwrite: 'auto' }, 0.1);
    }

    const onEmailBlur = () => resetFace();
    const onPasswordFocus = () => coverEyes();
    const onPasswordBlur = () => uncoverEyes();
    ...
    gsap.set(armL.current, { x: -93, y: 220, rotation: 105, transformOrigin: 'top left' });
    gsap.set(armR.current, { x: -93, y: 220, rotation: -105, transformOrigin: 'top right' });
  ...
  return (
    <div
      ref={svgContainerRef}
      className="relative w-[200px] h-[200px] mx-auto mb-4 rounded-full border-[2.5px] border-[#3A5E77] overflow-hidden pointer-events-none"
    >
      <svg className="absolute left-0 top-0 w-full h-full" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200">
        <defs>
        ...
        <path ref={chin} d="M84.1 121.6c2.7 2.9 6.1 5.4 9.8 7.5l.9-4.5c2.9 2.5 6.3 4.8 10.2 6.5 0-1.9-.1-3.9-.2-5.8 3 1.2 6.2 2 9.7 2.5-.3-2.1-.7-4.1-1.2-6.1" fill="none" stroke="#3a5e77" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
        <path ref={face} fill="#DDF1FA" d="M134.5,46v35.5c0,21.815-15.446,39.5-34.5,39.5s-34.5-17.685-34.5-39.5V46" />
        ...
        <g ref={mouth}>
          <path ref={mouthBG} fill="#617E92" d="M100.2,101c-0.4,0-1.4,0-1.8,0c-2.7-0.3-5.3-1.1-8-2.5c-0.7-0.3-0.9-1.2-0.6-1.8 c0.2-0.5,0.7-0.7,1.2-0.7c0.2,0,0.5,0.1,0.6,0.2c3,1.5,5.8,2.3,8.6,2.3s5.7-0.7,8.6-2.3c0.2-0.1,0.4-0.2,0.6-0.2 c0.5,0,1,0.3,1.2,0.7c0.4,0.7,0.1,1.5-0.6,1.9c-2.6,1.4-5.3,2.2-7.9,2.5C101.7,101,100.5,101,100.2,101z" />
          <path ref={mouthSmallBG} style={{ display: 'none' }} fill="#617E92" d="M100.2,101c-0.4,0-1.4,0-1.8,0c-2.7-0.3-5.3-1.1-8-2.5c-0.7-0.3-0.9-1.2-0.6-1.8 c0.2-0.5,0.7-0.7,1.2-0.7c0.2,0,0.5,0.1,0.6,0.2c3,1.5,5.8,2.3,8.6,2.3s5.7-0.7,8.6-2.3c0.2-0.1,0.4-0.2,0.6-0.2 c0.5,0,1,0.3,1.2,0.7c0.4,0.7,0.1,1.5-0.6,1.9c-2.6,1.4-5.3,2.2-7.9,2.5C101.7,101,100.5,101,100.2,101z" />
          <path ref={mouthMediumBG} style={{ display: 'none' }} d="M95,104.2c-4.5,0-8.2-3.7-8.2-8.2v-2c0-1.2,1-2.2,2.2-2.2h22c1.2,0,2.2,1,2.2,2.2v2 c0,4.5-3.7,8.2-8.2,8.2H95z" />
          <path ref={mouthLargeBG} style={{ display: 'none' }} d="M100 110.2c-9 0-16.2-7.3-16.2-16.2 0-2.3 1.9-4.2 4.2-4.2h24c2.3 0 4.2 1.9 4.2 4.2 0 9-7.2 16.2-16.2 16.2z" fill="#617e92" stroke="#3a5e77" strokeLinejoin="round" strokeWidth="2.5" />
          <defs>
            <path ref={mouthMaskPath} id="mouthMaskPath" d="M100.2,101c-0.4,0-1.4,0-1.8,0c-2.7-0.3-5.3-1.1-8-2.5c-0.7-0.3-0.9-1.2-0.6-1.8 c0.2-0.5,0.7-0.7,1.2-0.7c0.2,0,0.5,0.1,0.6,0.2c3,1.5,5.8,2.3,8.6,2.3s5.7-0.7,8.6-2.3c0.2-0.1,0.4-0.2,0.6-0.2 c0.5,0,1,0.3,1.2,0.7c0.4,0.7,0.1,1.5-0.6,1.9c-2.6,1.4-5.3,2.2-7.9,2.5C101.7,101,100.5,101,100.2,101z" />
          </defs>
          ...
          <path ref={tooth} clipPath="url(#mouthMask)" style={{ fill: '#FFFFFF' }} d="M106,97h-4c-1.1,0-2-0.9-2-2v-2h8v2C108,96.1,107.1,97,106,97z" />
          <path ref={mouthOutline} fill="none" stroke="#3A5E77" strokeWidth="2.5" strokeLinejoin="round" d="M100.2,101c-0.4,0-1.4,0-1.8,0c-2.7-0.3-5.3-1.1-8-2.5c-0.7-0.3-0.9-1.2-0.6-1.8 c0.2-0.5,0.7-0.7,1.2-0.7c0.2,0,0.5,0.1,0.6,0.2c3,1.5,5.8,2.3,8.6,2.3s5.7-0.7,8.6-2.3c0.2-0.1,0.4-0.2,0.6-0.2 c0.5,0,1,0.3,1.2,0.7c0.4,0.7,0.1,1.5-0.6,1.9c-2.6,1.4-5.3,2.2-7.9,2.5C101.7,101,100.5,101,100.2,101z" />
        </g>

        <path ref={nose} d="M97.7 79.9h4.7c1.9 0 3 2.2 1.9 3.7l-2.3 3.3c-.9 1.3-2.9 1.3-3.8 0l-2.3-3.3c-1.3-1.6-.2-3.7 1.8-3.7z" fill="#3a5e77" />
        ...
        <path ref={hair} fill="none" stroke="none" d="" />
      </svg>
    </div>
  );
};